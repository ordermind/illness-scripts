import Illness from "../Illness.js";

export default new Illness({
    id: "peripheral-arterial-disease",
    name: "Perifeer arterieel vaatlijden",
    images: [
        "./images/perfusion/peripheral_arterial_disease.jpg",
    ],
    pathophysiology: `
Perifeer arterieel vaatlijden is een door atherotrombose veroorzaakte klinische manifestatie van hart- en vaatziekte.

Ondercategorieën:
- Acute ischemie van het (onder)been: acute perfusiestoornis van een voet of onderbeen ten gevolge van snelle progressie van een plaque of een arteriële embolie, die binnen enkele uren tot dagen een bedreiging vormt voor de levensvatbaarheid daarvan
- Chronisch obstructief arterieel vaatlijden: geleidelijk progressieve perfusiestoornis van een voet of onderbeen, met 2 uitingsvormen:
    - claudicatio intermittens
    - kritieke ischemie

#### Comorbiditeiten

- diabetes mellitus
- chronisch obstructieve longziekten (COPD)
- hartfalen
- angina pectoris
- cerebrovasculair accident (CVA)
- artrose
- klachten van wervelkolom/SI-gewricht/heupgewricht/kniegewricht, voet(en)
    `,
    epidemiology: `
#### Acute ischemie

- Incidentie: 0,014%

#### Chronisch obstructief arterieel vaatlijden

- Prevalentie: 7% bij 55-jarigen, 56% bij personen ≥ 85 jaar
- Incidentie: 0,3%
- Prevalentie en incidentie van chronisch obstructief arterieel vaatlijden zijn vrijwel gelijk bij mannen en vrouwen, maar mannen ontwikkelen eerder claudicatio intermittens

##### Kritieke ischemie

- Prevalentie: 0,5% bij personen ≥ 60 jaar
- Incidentie: 0,05% - 0,1%

#### Risicofactoren

- Roken (belangrijkste factor)
- Leeftijd ≥ 50 jaar
- Doorgemaakte hart- en vaatziekten, diabetes mellitus, of reumatoïde artritis
- Geslacht (mannen hoger risico dan vrouwen)
- Familieanamnese (hart- en vaatziekten voor het 65e levensjaar bij ouders, broers of zusters)
- Voedingspatroon (afwijkend van de richtlijnen goede voeding)
- Overmatig alcoholgebruik (vrouwen meer dan 1-2 glazen/dag, mannen meer dan 2-3 glazen/dag)
- Gebrek aan lichamelijke activiteit (< 5 dagen per week 30 min/dag, bijvoorbeeld fietsen, stevig
wandelen of tuinieren)
- Donkere huidskleur

De kans op andere hart- en vaatziekten is 3x zo hoog als bij personen zonder chronisch obstructief arterieel vaatlijden
    `,
timeCourse: `
#### Chronisch obstructief arterieel vaatlijden

- Geleidelijk progressief
- Prognostische factoren:
    - leeftijd > 70 jaar
    - familiaire belasting voor hart- en vaatziekten
    - roken
    - ernstige klachten
    - aanwezigheid van vernauwingen op meerdere niveaus
    - coronaire en cerebrovasculaire aandoeningen
    - diabetes mellitus
    - lage enkel-armindex
`,
    symptoms: `
#### Acute ischemie

De symptomen van acute ischemie kunnen samengevat worden in de <strong>vijf P's</strong>:
- Pijn in rust
- Pallor (Bleekheid)
- Pulsloosheid
- Paralyse (spierzwakte / beginnende verlamming, vaak beginnend bij de intrinsieke voetspieren)
- Paresthesie (doof gevoel, tintelingen)

#### Claudicatio intermittens

- Klachten in de beenspieren (bil, dijbeen, kuit) die optreden bij lopen:
    - pijn (zelden hevig)
    - moe en stijf gevoel
    - krampen
- Bij stilstaan verdwijnen de klachten, maar ze komen terug als de patiënt verder loopt
- Als de patiënt sneller loopt of een heuvel op gaat, ontstaan de klachten eerder
- De patiënt heeft geen klachten als hij zit of staat
- Soms is er een eenzijdig koudegevoel in de voet

#### Kritieke ischemie

- Pijnklachten (meestal hevige pijn) aan voet of been, vooral in de voorvoet en aan de tenen
- In het begin heeft de patiënt alleen 's nachts last door het wegvallen van de zwaartekracht
- Bij veel patiënten vermindert de pijn als ze het been buiten het bed laten afhangen of als ze even uit bed gaan
- Verminderde huidcirculatie (zoals koude, dove voeten)
- Nagelafwijkingen
- Verminderde haargroei op tenen en onderbenen
- Slecht genezende huidwondjes
- Soms is de voet ook wat gezwollen en erythemateus
- Bij ernstigere vormen zijn er ulcera of is er (dreigende) necrose of gangreen aan de voet

    `,
    diagnostics: `
#### Fontaine classificatie van chronisch obstructief arterieel vaatlijden

- Fontaine I: asymptomatisch vaatlijden
- Fontaine II: inspanningsgebonden klachten
    - IIa: klachten bij lopen > 100m
    - IIb: klachten bij lopen < 100m
- Fontaine III: klachten in rust
- Fontaine IV: weefselverlies (ulcera, gangreen)

Bij Fontaine III en IV is er sprake van kritieke ischemie.

#### Observatie
- Mank lopen
- Minder belasting op het aangedane been zetten
- Slechte voetafwikkeling bij het aangedane been, een beetje plat lopen

#### Anamnese

- Vraag hoe lang de klachten bestaan
- Vraag of er (snelle) progressie is
- Besteed aandacht aan de invloed van de klachten op de kwaliteit van leven (<a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/euroqol-5d/">EQ-5D</a>, <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/vascular-quality-of-life-questionnaire/">VascuQol-25</a>)
- Vraag hoe groot de maximale loopafstand is (≤ 100 m?)
- Besteed aandacht aan risicofactoren voor hart- en vaatziekten (CVRM)
- Vraag naar behoefte aan informatie (vragenlijst informatiebehoefte, bijlage 2 van KNGF richtlijn voor sPAV)

##### Acute ischemie

- Vraag naar de vijf P's

##### Claudicatio intermittens

- Vraag naar pijn of andere vervelende sensaties (moe, stijf, krampen, temperatuurverschillen) in een
spiergroep van de bilregio of het been, die optreden bij inspanning en verminderen in rust
- Links-rechtsverschil

##### Kritieke ischemie

- Vraag naar rustpijn en/of nachtelijke pijn (vooral in de voorvoet of tenen), die afneemt als de patiënt
opstaat of het aangedane been laat hangen
- Vraag naar afwijkingen aan de huid of nagels van de voeten, zoals wondjes of zweertjes

#### Lichamelijk onderzoek

Let bij de beoordeling op links-rechtsverschillen.

##### Acute ischemie

- Ontbrekende arteriële pulsaties bij palpatie van a. tibialis posterior, a. dorsalis pedis en/of a. femoralis (palpeer de a. poplitea als de a. femoralis pulseert, maar palpaties over de voetarteriën ontbreken)
- Souffle bij auscultatie van de a. femoralis met behulp van de stethoscoop (let op dat te grote druk van de stethoscoop een artificiële souffle kan opwekken)
- Lagere huidtemperatuur van voet en onderbeen bij palpatie met de handrug
- Bleekheid van het been
- Gevoelsstoornissen van het been (vaak de interdigitale ruimte of de voetrug tussen 1e en 2e straal)
- Spierzwakte van het been (vaak zwakte of motorische uitval van de intrinsieke voetspieren)

##### Claudicatio intermittens

<table class="table table-striped">
    <tr><td>
Verzwakte arteriële pulsaties bij palpatie van a. tibialis posterior, a. dorsalis pedis en/of a. femoralis
    </td></tr>
    <tr><td>
Souffle bij auscultatie van de a. femoralis
    </td></tr>
    <tr><td>
Lagere huidtemperatuur van voeten en onderbenen bij palpatie met de handrug
    </td></tr>
    <tr><td>
Trofische stoornissen: beoordeel de huid (vooral van de tenen, met aandacht voor wondjes), nagels, beharing van voeten en onderbenen
    </td></tr>
    <tr><td>
Bepaal de enkel-armindex (EAI) bij klachten van claudicatio intermittens en/of als een van de bovenstaande onderzoeken een positief resultaat geeft. Een enkel-armindex in rust

Interpretatie van EAI onderzoek:

< 0,8 (eenmalige bepaling) of < 0,9 (gemiddelde van 3 bepalingen): chronisch obstructief arterieel vaatlijden vrijwel zeker (kans > 95%)

0,9-1,0: chronisch obstructief arterieel vaatlijden niet zeker, overweeg alternatieve diagnosen of verwijs voor nadere diagnostiek bij een aanhoudend vermoeden van chronisch obstructief arterieel vaatlijden

> 1,1 (eenmalige bepaling) of > 1,0 (gemiddelde van 3 bepalingen): chronisch obstructief
arterieel vaatlijden vrijwel uitgesloten (kans < 1%)
    </td></tr>
    <tr><td>
Evalueren van het looppatroon
    </td></tr>
    <tr><td>
<a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/loopbandtest/">Loopbandtest</a>:

Voorafgaand:
- <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/american-college-of-sport-medicin-schaal/">ACSM pijn</a> in rust
- Bloeddruk
- Hartfrequentie
- <a target="_blank" href="https://meetinstrumentenzorg.nl/wp-content/uploads/instrumenten/Borg-0-10-meetinstr.pdf">Borgscore voor vermoeidheid in de benen (0-10)</a>

Testprotocol:
- loopsnelheid 3,2 km/uur
- toename hellingshoek met 2% elke twee minuten beginnend bij 0%, tot maximaal 10%
- duur van de test maximaal 30 minuten

Patiënt geeft aan wanneer hij zou willen stoppen (functionele loopafstand).
Patiënt loopt door tot niet meer mogelijk (maximale loopafstand).

Uitkomstmaten:
- Functionele loopafstand (de afstand waarbij de patiënt zou willen stoppen) en <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/american-college-of-sport-medicin-schaal/">ACSM pijn</a> op dat moment
- Maximale loopafstand (de afstand waarbij de patiënt moet stoppen) en <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/american-college-of-sport-medicin-schaal/">ACSM pijn</a> op dat moment
- Afwijkingen in het looppatroon en moment van optreden ervan

Na afname van de loopbandtest wordt de loopafstand vastgelegd. Tevens wordt gelet op signalen die de loopafstand van de patiënt kunnen beperken:

- lokalisatie van de pijn
- ernst van de pijn (gebruik hiervoor de <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/american-college-of-sport-medicin-schaal/">ACSM schaal voor pijn</a>)
- uitputting
- pijn op de borst
- benauwdheid
- angst

    </td></tr>
    <tr><td>
Aanvullend functieonderzoek:

- vaststellen van de spierkracht
- mobiliteitsonderzoek, b.v. <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/walking-impairment-questionnaire/">Walking Impairment Questionnaire (WIQ)</a>
- evaluatie van coördinatie/stabiliteit
- sensibiliteitsonderzoek (in geval van diabetes mellitus)
    </td></tr>
    <tr><td>

Aanvullend onderzoek persoonlijke factoren:

- Inventarisatie van behoefte aan informatie over de aandoening - <a target="_blank" href="https://www.kngf.nl/binaries/content/assets/kennisplatform/onbeveiligd/richtlijnen/spav/downloads/spav-praktijkrichtlijn">KNGF vragenlijst informatiebehoefte (bijlage 2)</a>
- Inventarisatie van aanwezigheid van comorbiditeit - <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/cumulative-illness-rating-scale-voor-het-vastleggen-v-comorbiditeit/">CIRS</a>
- Effect op de kwaliteit van leven - <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/36-item-short-form-health-survey/">RAND-36</a>, <a target="_blank" href="https://meetinstrumentenzorg.nl/instrumenten/euroqol-5d/">EQ-5D</a>
    </td></tr>
</table>

##### Kritieke ischemie

- Systolische enkeldruk is < 50 mmHg.

#### Differentiaaldiagnostiek

<strong>Lumbale wervelkanaalstenose (neurogene claudicatio)</strong>, ook <strong>spinale stenose</strong> genoemd, is een vernauwing van het wervelkanaal waardoor zenuwen geklemd worden, en dat kan klachten veroorzaken tijdens het lopen zoals pijn in rug en/of benen, tintelingen en spierzwakte. In tegenstelling tot PAV heb je hierbij geen verminderde cirkulatie en dus ook geen kleurafwijkingen,  temperatuurverschillen of andere gerelateerde verschijnsels.

<strong>Perifere diabetische neuropathie</strong> treedt op wanneer de sensorische, motorische en autonome zenuwen van het perifere zenuwstelsel bij mensen met diabetes niet goed functioneren. Sommige patiënten ervaren pijnlijke sensorische neuropathie, waaronder een brandend of tintelend gevoel dat bekendstaat als paresthesie. In tegenstelling tot PAV heb je hierbij geen verminderde cirkulatie en dus ook geen kleurafwijkingen, temperatuurverschillen of andere gerelateerde verschijnsels.

<strong>Artrose</strong> veroorzaakt pijn in gewrichten terwijl bij PAV de pijn is niet beperkt tot gewrichten. Bij artrose heb je ook geen verminderde cirkulatie en dus ook geen kleurafwijkingen, temperatuurverschillen of andere gerelateerde verschijnsels.

<strong>Chronische veneuze insufficiëntie (CVI)</strong> betekent dat de venen niet meer goed functioneren, bijvooreeld vanwege <strong>varices (spataderen)</strong>. Meestal ontstaat een moe, loom en zwaar gevoel in de benen. Ook kan een vochtophoping rond de enkels ontstaan. Net als bij PAV kunnen klachten ontstaan tijdens lopen, maar de bloeddruk in de enkel zal hierbij niet lager zijn, dus een positieve EAI zou dit moeten kunnen uitsluiten.

Bij <strong>chronisch compartimentsyndroom</strong> ontstaat door er een te hoge druk in de spieren, na of tijdens sporten of wandelen, pijn en krampen in het onderbeen. Meestal zijn de klachten bilateraal terwijl PAV vaak unilateraal is. Je hebt hierbij ook geen trofische stoornissen zoals broze nagels of een dunne glanzende huid bij de voet.

Een <strong>Bakercyste</strong> veroorzaakt zwelling en pijn achter de knie, maar deze symptomen zijn niet specifiek gerelateerd aan inspanning en verbeteren niet met rust zoals bij PAV.
    `,
    treatment: `
#### Acute ischemie

<strong>Bij vermoeden op acute ischemie, verwijs met spoed naar een vaatchirurg (of bel 112)</strong>

- Trombolyse (oplossen van stolsel door een infuus van een krachtige bloedverdunner)
- Trombo-embolectomie (operatief verwijderen van een stolsel uit het bloedvat)

#### Claudicatio intermittens

- Educatie - geven van informatie en voorlichting ter bevordering van het ziekte-inzicht van de patiënt
- Cardiovasculaire risicomanagement
    - Stoppen met roken
- Verbeteren van het ( subjectieve en objectieve) inspanningsvermogen
    - het vergroten van de functionele loopafstand
    - het vergroten van het aeroob uithoudingsvermogen
    - het leren omgaan met de klachten door:
        - het verhogen van de pijntolerantie
        - het overwinnen van angst voor inspanning
- Optimaliseren van activiteiten en participatie van de patiënt

Looptraining - minimaal 30 minuten per sessie, afbouwend naar steeds meer zelfstandig
oefenen

Voorbeeld van een behandelschema:
<table class="table">
    <tr>
        <th>Fase</th>
        <th>Behandelfrequentie</th>
        <th>Duur fase</th>
        <th>Aantal behandelingen</th>
    </tr>
    <tr>
        <td>Startfase</td>
        <td>3 maal per week</td>
        <td>4 weken</td>
        <td>12</td>
    </tr>
    <tr>
        <td>Zelf-/trainingsfase</td>
        <td>1-2 maal per week</td>
        <td>8 weken</td>
        <td>8-16</td>
    </tr>
    <tr>
        <td>Onderhoudsfase</td>
        <td>1-2 maal per maand</td>
        <td>40 weken</td>
        <td>9-18</td>
    </tr>
</table>

Om de functionele loopafstand te vergroten:

Loopbandtraining onder begeleiding van fysiotherapeut (intervaltraining)
- 2-3 keer per week
- 30-50 min per sessie
- 2-6 maanden
- De weerstand moet zodanig aangepast worden dat matig tot ernstige pijn binnen 5-10 optreedt. Dit kan bereikt worden door de snelheid en helling aan te passen.
- Daarna 2-5 rusten totdat de pijn weer verdwenen is

Aeroob UHV training
of Kracht UHV training als andere vormen niet goed lukken

#### Kritieke ischemie

<strong>Bij vermoeden op kritieke ischemie, verwijs met spoed naar een vaatchirurg (of bel 112)</strong>

- Percutane Transluminale Angioplastiek (PTA, "dotteren")
- Bypass
- Endarteriëctormie
- Amputatie
    `,
    references: `
Bartelink, M.E.L., Elsman, B.H.P., Oostindjer, A., Stoffers, H.E.J.H., Wiersma, T.J., Geraets, J.J.X.R. (2014). <em>NHG-Standaard
Perifeer arterieel vaatlijden (M13)</em>. Nederlands Huisartsen Genootschap. Geraadpleegd op 15 maart 2024, van https://richtlijnen.nhg.org/files/pdf/79_Perifeer%20arterieel%20vaatlijden_februari-2014.pdf

Merry, A.H.H., Teijink, J.A.W., Jongert, M.W.A., Poelgeest, A., van der Voort, S.S.E.M., Bartelink, M.E.L., Stoffers, H.E.J.H., Brooymans, F., Hendriks, H.J.M., de Bie, R.A. (2014). <em>KNGF-richtlijn Symptomatisch perifeer arterieel vaatlijden</em>. Koninklijk Nederlands Genootschap voor Fysiotherapie (KNGF). Geraadpleegd op 15 maart 2024, van https://www.kngf.nl/binaries/content/assets/kennisplatform/onbeveiligd/richtlijnen/spav/downloads/spav-praktijkrichtlijn

Treat-Jacobson, D., McDermott, M. M., Bronas, U. G., Campia, U., Collins, T. C., Criqui, M. H., Gardner, A. W., Hiatt, W. R., Regensteiner, J. G., Rich, K., & American Heart Association Council on Peripheral Vascular Disease; Council on Quality of Care and Outcomes Research; and Council on Cardiovascular and Stroke Nursing (2019). Optimal Exercise Programs for Patients With Peripheral Artery Disease: A Scientific Statement From the American Heart Association. <em>Circulation, 139</em>(4), e10-e33. https://doi.org/10.1161/CIR.0000000000000623
    `
});
